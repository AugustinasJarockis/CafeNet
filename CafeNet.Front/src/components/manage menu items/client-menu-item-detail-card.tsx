...
export function ClientMenuItemDetailCard({
  ...
  const getTotalVariationPrice = () => {
    return selectedVariations.reduce(
      (sum, variation) => sum + variation.priceChange,
      0
    );
  };
    ...
    <Card className={`w-[450px] max-w-[90vw] max-h-[90vh] overflow-auto`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-semibold">
              {menuItem.tax.type} ({menuItem.tax.percent}%)
            </p>
          </div>
          <div>
            ...
            <p className="text-lg font-semibold">
              €
              {(
                (menuItem.price + getTotalVariationPrice()) *
                (menuItem.tax.percent / 100.0)
              ).toFixed(2)}
            </p>
          </div>
          <div>
            ...
            <p className="text-lg font-semibold">
              ...
              {(
                (menuItem.price + getTotalVariationPrice()) *
                (1.0 + menuItem.tax.percent / 100.0)
              ).toFixed(2)}
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center justify-between bg-muted/50 p-2 rounded-md">
            ...
            <SelectMenuItemVariationForm
              ...
              menuItemVariations={menuItem.menuItemVariations.filter(
                (item) =>
                  !selectedVariations
              ...
              {selectedVariations.map((variation, index) => (
                ...
                >
                  ...
                  <div>
                    <Badge
                      variant={
                        variation.priceChange >= 0 ? 'default' : 'destructive'
                      }
                    >
      ...
      <CardFooter className={'flex justify-between'}>
        <Button
          variant="default"
          onClick={() => {
            onAddToOrder(selectedVariations);
            onClose();
          }}
        >
          Add to order
